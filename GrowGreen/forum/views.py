...
def blog_post(request, post_id):
	if(request.method == 'POST'):
		...
		if(parent != ""):
			...
			obj=ForumComment(parent=parent_comment, user = user, message = message, subject=subject, post=post, timestamp = datetime.datetime.strptime('2021-09-09 00:00:00','%Y-%m-%d %H:%M:%S'))
		else:
			obj=ForumComment(user = user, message = message, subject=subject, post=post, timestamp = datetime.datetime.strptime('2021-09-09 00:00:00','%Y-%m-%d %H:%M:%S'))
		obj.save()
		email = user.email
		send_mail(
